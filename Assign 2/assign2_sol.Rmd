...
```
From the histogram graphs, we can see that the `bad`, `lawyers`, `employ`, `pop` have similar shapes, so there is a highly collinearity problem in linear regression.

### 5.a influence points 
We will use Cook distance and draw the corrponding plot chart to investigate influence points between expense data and other data. If the Cook's distance is bigger than 1, then it is considered to be an influence point.
```{r}
...
```
According to model expense~bad+crime+lawyers+employ+pop, we got 4 influence points in the following way:
    * Point 5 is an influence point, its Cook distance is 4.91 > 1.
    * Point 8 is an influence point, its Cook distance is 3.51 > 1.
    * Point 35 is an influence point, its Cook distance is 1.09 > 1.
    * Point 44 is an influence point, its Cook distance is 2.70 > 1.
    
### 5.a collinearity
We will use variance inflation factor(VIF) to investigate potential collinearity in the dataset, for the $\beta_j$ in model Y_n=$\beta_0+\beta_1X_{n1}+\beta_2X_{n2}+…+\beta_pX_{np}+e_n, n=1,2,…,N$,if the $\beta_j$'s VIF is bigger than 5, then it is considered  $\beta_j$ is unreliable.
```{r}
...
```
In the first model to third model only 1 out of 5 VIF is lower than 5, so there are collinearity problems in those models.
The last two models are ok with respect to collinearity problems. 

**b)**
We use summary function, and its eighth output is $R^2$ determination coefficient. We do not print all the results, but the p-value here is equality important.
...
```
Model expend~employ has max determination coefficient: 0.954, so we chose this model forthe next step.
```{r results='hide'}
...
```
In those four model only 'lawyers' in expend~employ+lawyers is significant(p-value=0.00113 < 0.05), and it has determination coefficient 0.9632 larger than 0.954, so wo chose this model for the next step.
```{r results='hide'}
...
```
All of these newly added features yield insignificant explanatory variables, so we can stop and take model expend~employ+lawyers as our final step-up model.
...
```
All remaining explanatory variables in the model are significant, so we can stop and take model expend~employ+lawyers as our final step-up model. Both methods generate the same model: expend~employ+lawyers.
...
```
There are V curve shown in the two charts, include $lawyers^2$ and $employ^2$
```{r warning=FALSE}
...
summary(expendlm2)
```
There is no specific curved pattern is visible, good!
