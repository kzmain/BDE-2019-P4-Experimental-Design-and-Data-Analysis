...
```
From box plot, we can see that `itermediate` and `warm` has similar declay time, `cold` can greatly increase declay time. Futhermore, middian time value of `dry` is bigger than `wet`.
From the interaction lines we can see that it do have interactions between envirnment and humidity.

**c)**
In this analysis thers's no mixed effect, so only do fixed effects analysis will be conducted.
```{r}
bread = read.table('bread.txt')
...
anova(breadlm)
```
Interest is in the main and interaction effects of the outer and inner factor. Main effects for environment and humidity are significant, at the same time the p-value of testing: $H_0 : \gamma_{i,j} = 0$ for all (i, j) is 3.705e-07,which means interaction effect is highly significant, in other word, the bread decay time are related to the effects between the humidity and the environment. If the humidity does not change, the environment changes then the decay time changes. If the environment does not change and humidity changes, so does the decay time.

**d)**
This is not a good question, from previous question's summary we also can see interaction effect is highly significan, we should NOT interpret the main effects without considering the interaction effect.
...
The Fitted plot seems to indicate that the residuals and the fitted values are uncorrelated. Conclusion: the second assumption of homogeneity of variance of the groups has been met. There are three outliers showed in the plot: NO.5, NO.7 and NO.8.
...
```
For block 1 assign unit 4 to treatment 1, unit 15 to treatment 2, etc., for block 2 assign unit 12 to treatment 1, unit 3 to treatment 2, etc.
...
```
As illusstrated for treatment, there's no significicant effect.
...
```
No, it is not a valid test for a difference in milk production. This is because when we use t-test for seeing effects, we aussume that there's no order effect, but from previous conclusion, there does have order effect.
Its conclusion compatible with the one obtained in a, because the result in a) is fixed effect analysis, it does not consider the order effect so does `t.test()` in this question. So the p-value for treatment is identical to the reuslt found in a). The p-value for id is not interesting.
...
```
From the histgram graphs, we can see that the `bad`, `lawyers`, `employ`, `pop` has similar shapes, so there's highly collinearity problem in linear regression.

### 5.a influence points 
We will use Cook distance and draw corrponding plot chart to investigate influence points between expense data and other data, if the Cook's distance is bigger than 1, then it is considered to be an influence point.
```{r}
...
```
According to model expense~bad+crime+lawyers+employ+pop, we got 4 inluence points as following:
...
### 5.a collinearity
We will use variance inflation factor(VIF) to investigate potentail collinearity in the dataset, for the $\beta_j$ in model Y_n=$\beta_0+\beta_1X_{n1}+\beta_2X_{n2}+…+\beta_pX_{np}+e_n, n=1,2,…,N$,if the $\beta_j$'s VIF is bigger than 5, then it is considered  $\beta_j$ is unreliable.
```{r}
...
```
In the 1st model to 3rd model only 1 out of 5 VIF is lower than 5, so there are collinearity problems in those model.
...
```
All remaining explanatory variables in the model are significant, so we can stop and take model expend~employ+lawyers as our final step-up model.Both method generate same model: expend~employ+lawyers.
...
```
There are V curve show in two charts, include $lawyers^2$ and $employ^2$
```{r warning=FALSE}
...
summary(expendlm2)
```
There's no specific curved pattern is visible, good!

(2)Plot residuals of X_j  against residuals of Y with omitted X_j 
```{r}
x=residuals(lm(lawyers+employ~bad+crime+pop)) 
y=residuals(lm(expend~bad+crime+pop))
plot(x,y)
```

(3)normal QQ-plot of the residuals
```{r}
plot(expendlm2, 2)
