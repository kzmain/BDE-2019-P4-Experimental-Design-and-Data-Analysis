...
```
After computing all three data sets' p-value, the data from light1879 and light1882 have a p-value greater than 0.05, and the data from light has a p-value lower than 0.05. So it can be deduced that 'light1879' and 'light1882' are both normally distributed, and the data set 'light' is not normally distributed.
...
**b)**
With lambda in the range between 0.01 and 0.1, we used a bootstrap-test to test every single one of them to evaluate whether thedata fits an exponential distribution.
```{r}
...
p
```
As result, when lambda equals 0.02, p-value = 0.074 > 0.05, so the data fits Exp(0.02).
